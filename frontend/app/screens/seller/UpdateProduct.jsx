...
const UpdateProduct = () => {
  ...
  const [price, setPrice] = useState((item.originalPrice).toString());
  const [quantity, setQuantity] = useState((item.quantityInStock).toString());
  const [category, setCategory] = useState(item.categoryId);
  const [imgURI, setImgURI] = useState(((item.images)[0]).url);
  ...
  const updateProduct = async () => {
    try {
      ...
      if (respone) {
        showMessage({
          message: 'Đã chỉnh sửa sản phẩm thành công',
          description: '',
          type: 'success',
          position: 'top',
          duration: 2000,
          ...
          <View>
            <View style={styles.headerContainer}>
              ...
              <View style={styles.header}>
                <Text style={styles.headerText}>Chỉnh sửa sản phẩm</Text>
              </View>
            </View>
